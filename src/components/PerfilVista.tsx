...
// ————————————— Datos demo (a conectar con las APIs de redes después) —————————————
const METRICAS = [
  { label: "Seguidores totales", valor: "128.4K", icon: <UsersIcon />, tono: "text-accent" },
  { label: "Vistas", valor: "2.3M", icon: <EyeIcon />, tono: "text-blue" },
  { label: "Engagement", valor: "8.7%", icon: <HeartIcon />, tono: "text-pink" },
  { label: "Interacciones", valor: "198.3K", icon: <ChatIcon />, tono: "text-accent" },
];
const PAISES = [
  { k: "MX", v: 7400, t: "7.4K" }, { k: "US", v: 903, t: "903" }, { k: "CO", v: 497, t: "497" },
  { k: "PE", v: 312, t: "312" }, { k: "VE", v: 259, t: "259" }, { k: "AR", v: 255, t: "255" },
];
const CIUDADES = [
  { k: "Mexico City, ...", v: 1200, t: "1.2K" }, { k: "Monterrey, N...", v: 448, t: "448" },
  { k: "Zapopan, Jal...", v: 330, t: "330" }, { k: "Lima, Lima R...", v: 215, t: "215" },
  { k: "Guadalajara, ...", v: 197, t: "197" }, { k: "Bogotá, Distri...", v: 126, t: "126" },
];
const GENERO = [
  { k: "Mujeres", v: 8000, t: "8.0K" }, { k: "Hombres", v: 1900, t: "1.9K" }, { k: "No especific...", v: 1100, t: "1.1K" },
];
const EDAD = [
  { k: "25-34", v: 5000, t: "5.0K" }, { k: "35-44", v: 3200, t: "3.2K" }, { k: "18-24", v: 1400, t: "1.4K" },
  { k: "45-54", v: 1100, t: "1.1K" }, { k: "55-64", v: 277, t: "277" }, { k: "13-17", v: 133, t: "133" },
];
const REDES = [
  { key: "tiktok", nombre: "TikTok", sub: "@alyssonugc", color: "#111827", icon: <TikTokIcon /> },
  { key: "instagram", nombre: "@alysson.ugc", sub: "", color: "grad-ig", icon: <InstagramIcon /> },
  { key: "youtube", nombre: "Alysson UGC", sub: "", color: "#FF0000", icon: <YouTubeIcon /> },
];

// ————————————— Componente principal —————————————
export function PerfilVista({ perfil, creadoEn }: { perfil: Perfil; creadoEn: string | null }) {
  const [tab, setTab] = useState<"Resumen" | "Métricas">("Métricas");
        ...
        <div className="max-w-[1240px] mx-auto px-4 sm:px-8 py-5">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6 items-start">
            ...
            <div className="min-w-0">
              ...
              <section className="rounded-3xl p-5 sm:p-6 shadow-sm border border-border"
                ...
                <div className="flex items-start gap-5">
                  <div className="relative shrink-0">
                    ...
                    <span className="absolute bottom-1.5 right-1.5 w-5 h-5 rounded-full bg-accent border-[3px] border-white" />
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        ...
                        <p className="text-sub text-sm mt-0.5">{edad ? `${edad} Años` : ""}</p>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 text-sub text-sm">
                        ...
                        <span className="text-xl leading-none">{banderaPais(perfil.pais)}</span>
                      </div>
                    </div>
              ...
              {tab === "Métricas" ? <TabMetricas /> : <TabResumen perfil={perfil} nivel={nivel} xpPct={xpPct} />}
            </div>
            ...
            <aside className="space-y-6 lg:sticky lg:top-5">
              ...
              <section className="bg-surface border border-border rounded-3xl p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-[11px] font-semibold rounded-full px-2.5 py-1 ${tieneRedes ? "text-sub bg-bg" : "text-sub bg-bg"}`}>
                    {tieneRedes ? "Conectadas" : "Conectar"}
                  </span>
                </div>
                <div className="space-y-4">
                  {REDES.map((r) => {
                    const conectado = !!perfil.redes?.[r.key] || true; // demo: mostrar conectadas como el mockup
                    return (
                      <div key={r.key} className="flex items-center gap-3">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {r.sub && <div className="text-[13px] text-sub truncate">{r.sub}</div>}
                        </div>
                        {conectado && (
  ...
  return (
    <div className="mt-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        <button className="flex items-center justify-between gap-6 bg-surface border border-border rounded-2xl px-5 py-3.5 shadow-sm min-w-[240px]">
          <span className="font-display font-extrabold">General</span>
          <ChevronDown />
        </button>
        <div className="flex items-center gap-2.5">
          <span className="w-9 h-9 rounded-xl grid place-items-center text-white" style={{ background: "linear-gradient(45deg,#F58529,#DD2A7B,#8134AF)" }}><InstagramIcon /></span>
          <span className="w-9 h-9 rounded-xl grid place-items-center text-white" style={{ background: "#1877F2" }}><FacebookIcon /></span>
          <span className="w-9 h-9 rounded-xl grid place-items-center text-white" style={{ background: "#111827" }}><TikTokIcon /></span>
        </div>
      </div>
        ...
        {METRICAS.map((m) => (
          <div key={m.label} className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
            <div className="flex items-center gap-2.5 mb-3">
              <span className={m.tono}>{m.icon}</span>
              <span className="text-sub font-medium text-[15px]">{m.label}</span>
            </div>
            <div className="font-display text-4xl font-extrabold tracking-tight">{m.valor}</div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Breakdown titulo="Países" icon={<PinIcon />} filas={PAISES} />
        <Breakdown titulo="Ciudades" icon={<BuildingIcon />} filas={CIUDADES} />
        <Breakdown titulo="Género" icon={<UsersIcon />} filas={GENERO} />
        <Breakdown titulo="Edad" icon={<BarsIcon />} filas={EDAD} />
      </div>
    </div>
  );
}
...
// ————————————— Tab Resumen —————————————
function TabResumen({ perfil, nivel, xpPct }: { perfil: Perfil; nivel: ReturnType<typeof nivelPorXP>; xpPct: number }) {
  const [rol, setRol] = useState<"booster" | "creador">("booster");
  const badges = [
    { emoji: "🔥", bg: "linear-gradient(135deg,#fb923c,#ea580c)" },
  ...
  const habilidades = [
    { nombre: "Creación de contenido", nivel: 78 }, { nombre: "Grabación y edición", nivel: 64 },
    { nombre: "Estrategia de redes", nivel: 55 }, { nombre: "Copywriting", nivel: 47 },
  ];
  return (
    <div className="mt-6 space-y-6">
      <div className="grid grid-cols-2 gap-3">
        {(["booster", "creador"] as const).map((r) => (
          <button key={r} onClick={() => setRol(r)}
            className={`h-12 rounded-2xl font-bold text-sm transition ${rol === r ? "bg-accent text-white shadow-sm" : "bg-surface border border-border text-sub hover:border-accent/40"}`}>
            {r === "booster" ? "Booster" : "Creador"}
          </button>
        ))}
      </div>

      <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
        <MiniStat top={<StatBars />} valor={`${nivel.actual.nivel}`} label="Nivel actual" />
        <MiniStat top={<span className="text-amber">⭐</span>} valor={perfil.xp.toLocaleString()} label="Puntos" />
        <MiniStat top={<span className="text-pink">📖</span>} valor={`0 / ${TOTAL_CLASES}`} label="Clases" />
        <MiniStat top={<span className="text-accent">💥</span>} valor={`0 / ${TOTAL_CLASES}`} label="Retos" />
        <MiniStat top={<span>🔥</span>} valor={`${perfil.racha}`} label="Días de racha" />
      </div>

      <div>
        <h3 className="font-display text-lg font-extrabold mb-3">Insignias</h3>
      ...
      <section className="bg-surface border border-border rounded-2xl p-5 shadow-sm">
        <h3 className="font-display text-lg font-extrabold mb-4">Habilidades</h3>
        <div className="space-y-4">
          {habilidades.map((h) => (
            <div key={h.nombre}>
              <p className="font-semibold text-sm mb-1.5">{h.nombre}</p>
              <div className="h-2 rounded-full bg-bg overflow-hidden">
                <div className="h-full rounded-full bg-gradient-to-r from-[#A78BFA] to-accent" style={{ width: `${h.nivel}%` }} />
              </div>
            </div>
          ))}
        </div>
        <p className="sr-only">{xpPct}</p>
      </section>
    </div>
  );
}
...
function MiniStat({ top, valor, label }: { top: React.ReactNode; valor: string; label: string }) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-4 text-center shadow-sm">
      <div className="grid place-items-center h-6 mb-1 text-lg">{top}</div>
